...
fun dynamicDarkColorScheme(): ColorScheme {
    val palettes = LocalTonalPalettes.current
    val amoledDarkTheme = AFSetting.useAmoLedDark
    ...
    return darkColorScheme(
        ...
        surface = palettes neutral if (AFSetting.useAmoLedDark) 0 else 10,
...
infix fun Color.onLight(lightColor: Color): Color =
    if (!AFSetting.useDarkTheme) lightColor else this

@Composable
infix fun Color.onDark(darkColor: Color): Color =
    if (AFSetting.useDarkTheme) darkColor else this
...
infix fun Color.alwaysLight(isAlways: Boolean): Color {
    ...
    return if (isAlways && AFSetting.useDarkTheme) {
        when (this) {
